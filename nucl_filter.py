...
#validating input
if len(sys.argv) != 5:
    print("Usage: python3 nucl_filter.py [fasta] [ID file] [min length] [max length]")
    sys.exit(1)
...
with open(fa1) as handle: 
    for record in SeqIO.parse(handle, "fasta"):
        if record.id in ID and len(record.seq) > x and len(record.seq) < y and record.seq[0]=='A':       #only take sequences of a certain length 
